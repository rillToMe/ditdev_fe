...
import type { XpResponse } from '../types/api'
...
const SKILLS: SkillNode[] = [
  ...
  { id: 'nodejs',     label: 'Node.js',       icon: SiNodedotjs,    category: 'web',      x: 0.69, y: 0.55, tier: 'advanced',     connects: ['javascript','react','postgresql'] },
...
const TIER_CONFIG: Record<Tier, { color: string; glow: string; label: string; size: number }> = {
...
function XPBar() {
  ...
  const [gainAnim, setGainAnim] = useState<string | null>(null)
  ...
  const draw = useCallback(() => {
    ...
    SKILLS.forEach(skill => {
      ...
      skill.connects.forEach(targetId => {
        ...
        const toHex   = (a: number) => Math.round(a * 255).toString(16).padStart(2, '0')
