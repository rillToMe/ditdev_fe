...
const CommitCard = ({ event, index }) => {
  ...
  const cfg = typeConfig[event.type] || { icon: <FiGithub />, color: 'text-pixel-gray', label: event.type?.replace('Event','') || 'EVENT' };
...
//Stats Row 
const StatPill = ({ value, label, color = 'pixel-blue' }) => (
  <div
...
export default function GitHubActivity() {
  ...
  const [tab,      setTab]      = useState('graph'); // 'graph' | 'commits'
  ...
  const pushEvents = events.filter(e => e.type === 'PushEvent').slice(0, 12);
            ...
            {tab === 'commits' && (
              ...
              >
                {allEvents.length === 0 ? (
                  ...
                  allEvents.map((ev, i) => (
                    <CommitCard key={ev.id} event={ev} index={i} />
                  ))
                )}
